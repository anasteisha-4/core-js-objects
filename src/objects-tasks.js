...
function sellTickets(queue) {
  ...
  queue.forEach((value) => {
    ...
    let diff = value - 25;
    if (diff > 0) {
      while (diff >= 100 && map[100] > 0) {
        diff -= 100;
        map[100] -= 1;
      }
      while (diff >= 50 && map[50] > 0) {
        diff -= 50;
        map[50] -= 1;
      }
      while (diff >= 25 && map[25] > 0) {
        diff -= 25;
        map[25] -= 1;
      }
      if (diff > 0) {
...
class Selector {
  ...
  checkOrder(type) {
    ...
    if (this.lastType && order.indexOf(this.lastType) > order.indexOf(type)) {
      throw new Error(
